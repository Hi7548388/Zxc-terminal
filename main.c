/*  Zxc Terminal — Full Featured Version with Themes
 *
 *  Features:
 *   - Tabs (GNOME Terminal style)
...
typedef struct {
    ...
    GKeyFile  *theme_file;
    GdkRGBA    current_bg;      /* current background color (without forced alpha) */
    gchar     *current_theme;   /* name of active theme (group) */
    double     current_alpha;   /* current transparency value [0.0 - 1.0] */
} AppWidgets;
...
create_terminal_tab(AppWidgets *app)
{
    ...
    gint page = gtk_notebook_append_page(GTK_NOTEBOOK(app->notebook), scroll, label);
...
/* -----------------------------------------------------------
   Font selector (modern VTE using Pango)
   ----------------------------------------------------------- */
static void
...
activate(GtkApplication *app, gpointer user_data)
{
    ...
    /* If themes are available, pick the first group as default and load it */
    ...
    /* If we have a default theme, set active row and emit changed once */
